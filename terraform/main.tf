...
resource "aws_s3_bucket" "storage" { bucket = "cloud-project-26-final" }
resource "aws_sqs_queue" "queue" { name = "nodebase-notification-queue" }
resource "aws_ecr_repository" "repo" { name = "nodebase-app-repo" }
resource "aws_cloudwatch_log_group" "ecs_logs" { name = "/ecs/nodebase-app" }
...
resource "aws_iam_role_policy_attachment" "exec_attach" { role = aws_iam_role.ecs_exec.name; policy_arn = "arn:aws:iam::aws:policy/service-role/AmazonECSTaskExecutionRolePolicy" }
...
resource "aws_iam_role_policy" "task_p" {
  ...
  policy = jsonencode({ Version = "2012-10-17", Statement = [{ Action = ["s3:*", "sqs:*"], Effect = "Allow", Resource = "*" }] })
}
...
resource "aws_lb_target_group" "tg" {
  ...
  vpc_id = aws_vpc.main.id
  target_type = "ip"
  health_check { path = "/api/auth/session" }
}
...
resource "aws_ecs_cluster" "cluster" { name = "nodebase-cluster-final" }
...
resource "aws_ecs_task_definition" "task" {
  ...
  container_definitions = jsonencode([{
    ...
    logConfiguration = {
      logDriver = "awslogs"
      options = { "awslogs-group" = aws_cloudwatch_log_group.ecs_logs.name, "awslogs-region" = var.region, "awslogs-stream-prefix" = "ecs" }
    }
  }])
}
